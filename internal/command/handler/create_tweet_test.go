...
func TestCreateTweet(t *testing.T) {
	...
	t.Run("正常にTweetを作成できる_140文字_マルチバイト文字", func(t *testing.T) {
		...
		// 取得して確認
		require.NoError(t, err)
		require.Len(t, tweetResp.GetId(), testutil.UUIDLength)
	...
	t.Run("AuthorIDに空文字を指定するとエラーが返る", func(t *testing.T) {
		...
		testutil.AssertGRPCError(t, err, codes.InvalidArgument, "author_id is required")
	})
	...
	t.Run("Bodyに空文字を指定するとエラーが返る", func(t *testing.T) {
		...
		require.Nil(t, tweetResp)
		testutil.AssertGRPCError(t, err, codes.InvalidArgument, "body is required")
	})
	...
	t.Run("AuthorIDにuuid形式でない文字列を指定するとエラーが返る", func(t *testing.T) {
		...
		require.Nil(t, tweetResp)
		testutil.AssertGRPCError(t, err, codes.InvalidArgument, "invalid UUID length: 8")
	})
	...
	t.Run("存在しないユーザーを指定するとエラーが返る", func(t *testing.T) {
		...
		require.Nil(t, tweetResp)
		testutil.AssertGRPCError(t, err, codes.NotFound, "user not found")
	})
	...
	t.Run("NewTweetからエラーが返る引数を指定するとエラーが返る", func(t *testing.T) {
		...
		require.Nil(t, tweetResp)
		testutil.AssertGRPCError(t, err, codes.InvalidArgument, "body is too long")
	})
}
